...
class DownloaderYtdl(DownloaderAbstract):
	...
	def _setup_ui(self):
		self.com.set_pbar_max_signal.emit(100)
	...
	def ytdl_processing_hook(self, d):
		...
		if d[self._ytdl.Keys.status] == self._ytdl.Keys.downloading:
			...
			if self._ytdl.Keys.total_bytes in d:
				...
				self.com.set_pbar_value_signal.emit(floor(downloaded / total * 100))
			...
			self.com.ready_for_playback_signal.emit(d[self._ytdl.Keys.filename])
